...
class MyGeneratorDecoder(nn.Module):
    ...
    def forward(self, app, com):
        ...
        # x = self.conv9(x)
        # x = self.norm9(x)
        # x = self.do_pad(app[6], x)
        # x = torch.cat([x, app[6], com[6]], 1)
        x = torch.cat([app[6], com[6]], 1)
        
        x = self.relu10(x)
        x = self.conv10(x)
        x = self.norm10(x)
        x = self.do_pad(app[5], x)
        x = torch.cat([x, app[5], com[5]], 1)
        
        x = self.relu11(x)
        x = self.conv11(x)
        x = self.norm11(x)
        x = self.do_pad(app[4], x)
        x = torch.cat([x, app[4], com[4]], 1)
        
        x = self.relu12(x)
        x = self.conv12(x)
        x = self.norm12(x)
        x = self.do_pad(app[3], x)
        x = torch.cat([x, app[3], com[3]], 1)
        
        x = self.relu13(x)
        x = self.conv13(x)
        x = self.norm13(x)
        x = self.do_pad(app[2], x)
        x = torch.cat([x, app[2], com[2]], 1)
        
        x = self.relu14(x)
        x = self.conv14(x)
        x = self.norm14(x)
        x = self.do_pad(app[1], x)
        x = torch.cat([x, app[1], com[1]], 1)
        
        x = self.relu15(x)
        x = self.conv15(x)
        x = self.norm15(x)
        x = self.do_pad(app[0], x)
